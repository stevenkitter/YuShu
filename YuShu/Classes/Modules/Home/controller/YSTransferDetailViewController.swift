...
//最新或者是建议
class YSPostSuggestDetailViewController: RootViewController {
    ...
    let headView = YSPostSuggestDetailHeadView.default()!
    ...
    func setupUI() {
        ...
        let h = UIView.transferDetailHeadViewH(transfer: postSuggest)
        tableViewHeader.setHeight(h: h)
    ...
    override func loadServerData() {
        ...
        guard let userId = UserManager.shareUserManager.curUserInfo?.user_id else {return}
        //评论
        NetworkManager.providerHomeApi.request(.getCommentList(user_id: userId, type: "post", item_id: transferId, page: page))
            .mapArray(Comment.self).subscribe(onNext: { (list) in
...
//MARK: 评论操作
extension YSPostSuggestDetailViewController{
    func like(btn bt: UIButton) {
        guard let userId = UserManager.shareUserManager.curUserInfo?.user_id else {return}
        WXActivityIndicatorView.start()
    ...
    func send() {
        guard let userId = UserManager.shareUserManager.curUserInfo?.user_id else {return}
        WXActivityIndicatorView.start()
        NetworkManager.providerHomeApi.request(.doComment(user_id: userId, comment_type: "post", comment_item_id: transferId, comment_desc: commentView.textField.text)).mapJSON().subscribe(onNext: { (res) in
            WXActivityIndicatorView.stop()
            guard let respon = res as? Dictionary<String, Any> else{
                return
            }
...
extension YSPostSuggestDetailViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return contents.count
    }
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
